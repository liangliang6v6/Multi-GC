...
class GCN(nn.Module):
    ...
    def _train_with_val(self, labels, data, train_iters, verbose, adj_val=False):
        ...
        # print(adj_full_norm, adj_full_norm.shape)
        ...
        for i in range(train_iters):
            ...
            with torch.no_grad():
                ...
                if adj_val:
                    loss_val = nn.BCEWithLogitsLoss()(output, labels_val.float())
                    acc_val, f1_macro, f1_weight = ml_acc(output, labels_val)
                else:
    ...
    def test(self, idx_test):
        ...
        self.eval()
        output = self.predict()
        loss_test = nn.BCEWithLogitsLoss()(output[idx_test], self.labels[idx_test].float())
        acc_test, f1_macro, f1_weight = ml_acc(output[idx_test], self.labels[idx_test])
        print("Test set results:",
              "loss= {:.4f}".format(loss_test.item()),
              "F1-micro= {:.4f}".format(acc_test.item()))
    ...
    def _train_with_val2(self, labels, idx_train, idx_val, train_iters, verbose):
        ...
        for i in range(train_iters):
            ...
            optimizer.zero_grad()
            output = self.forward(self.features, self.adj_norm)
            loss_train = nn.BCEWithLogitsLoss()(output[idx_train], labels[idx_train].float())
            loss_train.backward()
            optimizer.step()
            ...
            self.eval()
            output = self.forward(self.features, self.adj_norm)
            loss_val = nn.BCEWithLogitsLoss()(output[idx_val], labels[idx_val].float())
            acc_val, f1_macro, f1_weight = ml_acc(output[idx_val], labels[idx_val])
            if acc_val >= best_acc_val:
                best_acc_val = acc_val
                self.output = output
                weights = deepcopy(self.state_dict())
